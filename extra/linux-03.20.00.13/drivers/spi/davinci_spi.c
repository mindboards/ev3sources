...
static int davinci_spi_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "Reset In/OUT SPI module\n");
	...
	dev_info(&pdev->dev, "Set up SPIPC0.  CS and ENA init is done in davinci_spi_setup\n");
	spipc0 = SPIPC0_DIFUN_MASK | SPIPC0_DOFUN_MASK | SPIPC0_CLKFUN_MASK;
	iowrite32(spipc0, dspi->base + SPIPC0);

	/* initialize chip selects */
	dev_info(&pdev->dev, "initialize chip selects\n");
	if (pdata->chip_sel) {
	...
	/* master mode default */
	dev_info(&pdev->dev, "master mode default \n");

	set_io_bits(dspi->base + SPIGCR1, SPIGCR1_CLKMOD_MASK);
	set_io_bits(dspi->base + SPIGCR1, SPIGCR1_MASTER_MASK);
	set_io_bits(dspi->base + SPIGCR1, SPIGCR1_POWERDOWN_MASK);
	dev_info(&pdev->dev, "master mode default end \n");

	ret = spi_bitbang_start(&dspi->bitbang);
	dev_info(&pdev->dev, "spi_bitbang_start \n");
	if (ret)
		goto free_dma;
